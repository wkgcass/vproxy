/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class vproxy_vfd_windows_GeneralWindows */

#ifndef _Included_vproxy_vfd_windows_GeneralWindows
#define _Included_vproxy_vfd_windows_GeneralWindows
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     vproxy_vfd_windows_GeneralWindows
 * Method:    tapNonBlockingSupported
 * Signature: ()Z
 */
JNIEXPORT jboolean JNICALL Java_vproxy_vfd_windows_GeneralWindows_tapNonBlockingSupported
  (JNIEnv *, jobject);

/*
 * Class:     vproxy_vfd_windows_GeneralWindows
 * Method:    allocateOverlapped
 * Signature: ()J
 */
JNIEXPORT jlong JNICALL Java_vproxy_vfd_windows_GeneralWindows_allocateOverlapped
  (JNIEnv *, jobject);

/*
 * Class:     vproxy_vfd_windows_GeneralWindows
 * Method:    releaseOverlapped
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_vproxy_vfd_windows_GeneralWindows_releaseOverlapped
  (JNIEnv *, jobject, jlong);

/*
 * Class:     vproxy_vfd_windows_GeneralWindows
 * Method:    createTapHandle
 * Signature: (Ljava/lang/String;)J
 */
JNIEXPORT jlong JNICALL Java_vproxy_vfd_windows_GeneralWindows_createTapHandle
  (JNIEnv *, jobject, jstring);

/*
 * Class:     vproxy_vfd_windows_GeneralWindows
 * Method:    closeHandle
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_vproxy_vfd_windows_GeneralWindows_closeHandle
  (JNIEnv *, jobject, jlong);

/*
 * Class:     vproxy_vfd_windows_GeneralWindows
 * Method:    read
 * Signature: (JLjava/nio/ByteBuffer;IIJ)I
 */
JNIEXPORT jint JNICALL Java_vproxy_vfd_windows_GeneralWindows_read
  (JNIEnv *, jobject, jlong, jobject, jint, jint, jlong);

/*
 * Class:     vproxy_vfd_windows_GeneralWindows
 * Method:    write
 * Signature: (JLjava/nio/ByteBuffer;IIJ)I
 */
JNIEXPORT jint JNICALL Java_vproxy_vfd_windows_GeneralWindows_write
  (JNIEnv *, jobject, jlong, jobject, jint, jint, jlong);

#ifdef __cplusplus
}
#endif
#endif
