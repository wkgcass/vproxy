...
public class Mirror {
    ...
    private static List<EthernetPacket> formatPacket(MirrorContext ctx,
        ...
        for (ByteArray data : dataList) {
            ...
            {
                ByteArray pseudoHeader = SwitchUtils.buildPseudoIPv6Header(ip, Consts.IP_PROTOCOL_TCP, tcpPacketBytes.length());
                var foo = pseudoHeader.concat(tcpPacketBytes);
                int cksm = SwitchUtils.calculateChecksum(foo, foo.length());
