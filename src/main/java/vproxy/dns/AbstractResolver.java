...
public abstract class AbstractResolver implements Resolver {
    ...
    protected static List<InetSocketAddress> getNameServers() {
        ...
        try {
            ...
            while (true) {
                ...
                if (Utils.isIpLiteral(line)) {
                    try {
                        ret.add(new InetSocketAddress(InetAddress.getByName(line), 53));
    ...
    protected static Map<String, InetAddress> getHosts() {
    ...
    static Resolver getDefault() {
        ...
        synchronized (AbstractResolver.class) {
            if (defaultResolver != null)
                return defaultResolver;
            List<InetSocketAddress> nameServers = getNameServers();
            try {
                String name = "Resolver";
                if (nameServers.isEmpty()) {
                    defaultResolver = new JDKResolver(name);
                } else {
                    defaultResolver = new VResolver(name, nameServers, getHosts());
                }
            } catch (IOException e) {
