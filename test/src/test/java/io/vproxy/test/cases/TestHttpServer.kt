...
class TestHttpServer {
  companion object {
    ...
    fun beforeClass() {
      val serverSock = io.vproxy.base.connection.ServerSock.create(
      ...
      loop.loop { io.vproxy.base.util.thread.VProxyThread.create(it, "test-http-server") }
  ...
  private fun requestRaw(uri: String): io.vproxy.base.processor.http1.entity.Response {
    val response = loop!!.execute {
      val client = CoroutineHttp1ClientConnection.create(io.vproxy.vfd.IPPort("127.0.0.1", port))
      client.get(uri).send()
